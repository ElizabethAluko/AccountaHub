...
function Navigation({ openLoginModal, user, handleLogout }) {
    ...
    <nav className="bg-blue-900 p-4">
      <div className="max-w-screen-xl mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-6">
          ...
          <button onClick={openLoginModal} className="text-white hover:underline">Login</button>
        </div>
      </div>
	  ...
	  <Link onClick={openLoginModal} className="block text-white py-2">Login</Link>
        </div>
      )}
    </nav>
  );
